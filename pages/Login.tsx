  ...
  const handleAuth = async (role: UserRole) => {
    ...
    try {
      ...
      if (mode === 'REGISTER') {
        ...
        // SQL trigger handles profile creation, but we login the user immediately in the UI state
        onLogin({
          id: data.user.id,
          name: fullName,
          email: email,
      ...
      if (e.message?.toLowerCase().includes('fetch')) {
        setError("Connection Failed: Ensure your Supabase URL and API Key are configured in the project settings.");
      } else {
    ...
    <div className="min-h-screen bg-[#0f172a] flex flex-col items-center justify-center p-6 relative font-sans overflow-hidden">
      ...
      <div className="max-w-xl w-full relative z-10 py-10">
        <div className="text-center mb-10">
          ...
          <p className="text-slate-400 mt-3 font-medium tracking-wide">Unified Secure Academic Authentication</p>
        </div>
        ...
        <div className="bg-white/5 backdrop-blur-3xl border border-white/10 rounded-[3rem] p-10 shadow-2xl">
          ...
          <div className="space-y-4">
            {mode === 'REGISTER' && (
              <>
                <div className="relative group">
                  ...
                  <Users size={18} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-500" />
                </div>
                <div className="relative group">
                  <input type="text" placeholder="Institution Name" value={collegeName} onChange={e => setCollegeName(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 px-6 text-white outline-none focus:border-indigo-500" />
                  <Building2 size={18} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-500" />
                </div>
                <div className="relative group">
                  <input type="text" placeholder="Department" value={department} onChange={e => setDepartment(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 px-6 text-white outline-none focus:border-indigo-500" />
                  <BookOpen size={18} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-500" />
                </div>
              </>
            )}
            
            <div className="relative group">
              <input type="email" placeholder="Campus/Personal Email" value={email} onChange={e => setEmail(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 px-6 text-white outline-none focus:border-indigo-500 transition-all font-medium" />
            ...
            <div className="relative group">
              ...
              <Key size={18} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-500" />
            </div>
          ...
          {!selectedNetwork ? (
            <div className="grid grid-cols-1 gap-4 mt-8">
              <button onClick={() => validate() && setSelectedNetwork('EDU')} className="group flex items-center gap-6 p-6 bg-white/5 border border-white/10 rounded-[2rem] hover:border-indigo-500 hover:bg-white/10 transition-all text-left">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-slate-500 uppercase font-black tracking-widest mt-1">Full Academic Access</p>
                </div>
                <ChevronRight size={20} className="text-slate-700 group-hover:text-indigo-500 transition-all" />
              </button>
              <button onClick={() => validate() && setSelectedNetwork('GENERAL')} className="group flex items-center gap-6 p-6 bg-white/5 border border-white/10 rounded-[2rem] hover:border-slate-400 hover:bg-white/10 transition-all text-left">
            ...
            <div className="mt-8 space-y-6">
              ...
              {selectedNetwork === 'EDU' && mode === 'REGISTER' && (
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="col-span-1">
                    <label className="block text-[10px] font-black text-slate-500 uppercase mb-2">Major</label>
                    <input type="text" placeholder="e.g. AI" value={specialization} onChange={e => setSpecialization(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl py-3 px-4 text-white outline-none focus:border-indigo-500" />
                  </div>
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-4">
                {selectedNetwork === 'EDU' ? (
                  ...
                  <button onClick={() => handleAuth(UserRole.OUTSIDER)} disabled={isLoading} className="col-span-2 flex items-center justify-center gap-4 p-6 bg-white/5 border border-white/10 rounded-[2rem] hover:bg-indigo-600 transition-all text-white font-black text-sm uppercase tracking-[0.2em] disabled:opacity-50">
          ...
          <p className="mt-10 text-center text-slate-600 text-[9px] uppercase font-black tracking-[0.3em]">Institutional Merit Ledger Protected • Yugastr 2026</p>
        </div>
      </div>
    </div>
  );
